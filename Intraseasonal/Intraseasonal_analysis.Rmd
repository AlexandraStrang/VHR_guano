---
title: "Intra-seasonal guano variation analysis"
...
NEEDS UPDATING (OCT 2025)

This document outlines the analysis part of "Intra-seasonal variation and factors influencing satellite-derived population indices of Ad\'{e}lie penguins (*Pygoscelis adeliae*) in the Ross Sea", the first data chapter of Alexandra Strang's doctoral thesis. This document focuses on the first question of this chapter: Is there a pattern in the size of the guano stain within a breeding season, as estimated through satellite imagery? With this question, we aimed to test the assumption that guano area estimates from images taken anywhere between December and February are equivalent. We hypothesised that the size of guano stain observed in VHR imagery will increase as the movement of birds throughout the colony increases as the season progresses. We also aimed to investigate the influence of satellite-related factors on the variability of guano area estimates within a single breeding season. We analysed the change in guano area within a season and the rate of change, where a trend in guano area size indicates that seasonal variation in guano area needs to be accounted for when deriving population estimates from VHR imagery. This will insure accurate comparisons of colony size between different years.

All data scripts can be found at {INSERT GITHUB LINK to VHR_guano repo}
...
## 3.0 Study colonies and covariates

To test whether estimates of the guano stain differ in size within a breeding season, we focus on three large Ross Sea Ad\'{e}lie penguin colonies; capes Adare, Crozier, and Hallett. EXPAND

We looked at change in guano area over time and the influence of satellite-related variables. To explore the change in guano area over time within a breeding season we used the number of days since December 1st (ranging from 0 - 90 days) as the VHR imagery was taken between December and February. We explored the influence of factors related to how the satellite imagery was taken by extracting these variables from the imagery metadata. These satellite-related variables included the average sun elevation angle and sun azimuth, ground resolution, and off-nadir angle.

```{r clean-organsise-data, results='hide'}
...
Here we plot both the raw and log estimates of gunao area over the number of days since December first, for each colony and together. We also compute a table containing the means and standard deviations of within-season guano area at each colony.

### 4.1 Plot guano area by colony
...
### 4.2 Plot log guano area by colony
...
### 4.3 Table of guano area estimates

```{r table-guano-area, message=FALSE}
...
## 5.0 Satellite-related covariates

### 5.1 Checking for multi-colinearity
...
base_model <- lm(Dataset.1.4$GA ~ Dataset.1.4$Day_D1 + Dataset.1.4$MEANCOLLECTEDGSD + Dataset.1.4$MEANSUNAZ + Dataset.1.4$MEANSUNEL + Dataset.1.4$MEANOFFNADIRVIEWANGLE)
...
anova(log_base_lmm)
check_collinearity(log_base_lmm) # can't use vif for lmm
...
### 5.3 Build candidate models

We tested a series of candidate models that were subsets of a full model and excluded collinear variables.
...
The null model ranked higher than the models with satellite-related covariates, revealing that the variability in guano area can not be explained by these covariates given our data.

## 6.0 Test for February growth

We further investigated seasonal change by separating estimates of guano area into February estimates and estimates prior to February. EXPAND

### 6.1 Include February variable
```{r february-effect}
...
Given the 15 within-season guano area estimates, the best model included the binary February effect, ranking higher than the null model.

### 6.2 Check for February effect in guano area ~ breeding pair count model
```{r MSc-model}
...
Given 38 observations of concurrent guano area and breeding pair estimates across 14 Ross Sea colonies and images of varying dates between December and February, there was no difference between estimates taken in February compared to prior to February. Therefore, the February effect was not consistent outside of the 15 within-season images.
