# m10: 25,0.9 and 0.01,0.01, mesh sub 6
...
# model path
m10_path <- file.path(results_path, "m10")

# ensure model folder exists
if (!dir.exists(m10_path)) dir.create(m10_path, recursive = TRUE)
...
print("running guano model with 25, 0.9 range prior and 0.01, 0.01 sigma prior and mesh sub 6")
...
ggsave(file.path(m10_path, "G_Intensity_plot.png"), G_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

print("running GS model with 25, 0.9 range prior and 0.01, 0.01 sigma prior and mesh sub 6")
...
ggsave(file.path(m10_path, "GS_Intensity_plot.png"), GS_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

# save model outputs
saveRDS(null_model, file = file.path(m10_path, "null_model.rds"))
saveRDS(GS_model, file = file.path(m10_path, "GS_model.rds"))
saveRDS(G_model, file = file.path(m10_path, "G_model.rds"))
