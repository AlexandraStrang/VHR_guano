# m11: 25,0.9 and 0.1,0.01, mesh sub 9
...
# model path
m11_path <- file.path(results_path, "m11")

# ensure model folder exists
if (!dir.exists(m11_path)) dir.create(m11_path, recursive = TRUE)
...
mesh_sub <- fm_subdivide(Crozier_mesh,9)
print(mesh_sub$n)
...
ggsave(file.path(m11_path, "G_Intensity_plot.png"), G_Intensity_plot, 
...
print("running GS model with 25, 0.9 range prior and 0.1, 0.01 sigma prior and mesh sub 9")
...
ggsave(file.path(m11_path, "GS_Intensity_plot.png"), GS_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

# save model outputs
saveRDS(null_model, file = file.path(m11_path, "null_model.rds"))
saveRDS(GS_model, file = file.path(m11_path, "GS_model.rds"))
saveRDS(G_model, file = file.path(m11_path, "G_model.rds"))
