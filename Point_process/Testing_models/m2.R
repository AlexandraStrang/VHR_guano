...
# model path
m2_path <- file.path(results_path, "m2")

# ensure model folder exists
if (!dir.exists(m2_path)) dir.create(m2_path, recursive = TRUE)
...
ggsave(file.path(m2_path, "G_Intensity_plot.png"), G_Intensity_plot, 
...
ggsave(file.path(m2_path, "GS_Intensity_plot.png"), GS_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

# save model outputs
saveRDS(null_model, file = file.path(m2_path, "null_model.rds"))
saveRDS(GS_model, file = file.path(m2_path, "GS_model.rds"))
saveRDS(G_model, file = file.path(m2_path, "G_model.rds"))
