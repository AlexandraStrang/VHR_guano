# m8: 25,0.9 and 0.1,0.01, mesh sub 6
...
# model path
m8_path <- file.path(results_path, "m8")

# ensure model folder exists
if (!dir.exists(m8_path)) dir.create(m8_path, recursive = TRUE)
...
ggsave(file.path(m8_path, "G_Intensity_plot.png"), G_Intensity_plot, 
...
ggsave(file.path(m8_path, "GS_Intensity_plot.png"), GS_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

# save model outputs
saveRDS(null_model, file = file.path(m8_path, "null_model.rds"))
saveRDS(GS_model, file = file.path(m8_path, "GS_model.rds"))
saveRDS(G_model, file = file.path(m8_path, "G_model.rds"))
