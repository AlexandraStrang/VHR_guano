# m12: 10m covariates and 30m max.edge
...
# model path
m12_path <- file.path(results_path, "m12")

# ensure model folder exists
if (!dir.exists(m12_path)) dir.create(m12_path, recursive = TRUE)
...
ggsave(file.path(m12_path, "null_Intensity_plot.png"), null_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)
...
ggsave(file.path(m12_path, "G_Intensity_plot.png"), G_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)
...
ggsave(file.path(m12_path, "GS_Intensity_plot.png"), GS_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

# save model outputs
saveRDS(null_model, file = file.path(m12_path, "null_model.rds"))
saveRDS(GS_model, file = file.path(m12_path, "GS_model.rds"))
saveRDS(G_model, file = file.path(m12_path, "G_model.rds"))
