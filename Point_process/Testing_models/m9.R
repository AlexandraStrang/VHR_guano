# m9: fixed effect priors and 25,0.9 and 0.1,0.01
...
# model path
m9_path <- file.path(results_path, "m9")

# ensure model folder exists
if (!dir.exists(m9_path)) dir.create(m9_path, recursive = TRUE)
...
ggsave(file.path(m9_path, "G_Intensity_plot.png"), G_Intensity_plot, 
...
ggsave(file.path(m9_path, "GS_Intensity_plot.png"), GS_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

# save model outputs
saveRDS(null_model, file = file.path(m9_path, "null_model.rds"))
saveRDS(GS_model, file = file.path(m9_path, "GS_model.rds"))
saveRDS(G_model, file = file.path(m9_path, "G_model.rds"))
