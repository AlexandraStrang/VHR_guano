...
# model path
m6_path <- file.path(results_path, "m6")

# ensure model folder exists
if (!dir.exists(m6_path)) dir.create(m6_path, recursive = TRUE)
...
print("running guano model with 100, 0.9 range prior and 0.1, 0.01 sigma prior and mesh sub 3")
...
ggsave(file.path(m6_path, "G_Intensity_plot.png"), G_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

print("running GS model with 100, 0.9 range prior and 0.1, 0.01 sigma prior and mesh sub 3")
...
ggsave(file.path(m6_path, "GS_Intensity_plot.png"), GS_Intensity_plot, 
       width = 8, height = 5, units = "in", 
       dpi = 600)

# save model outputs
saveRDS(null_model, file = file.path(m6_path, "null_model.rds"))
saveRDS(GS_model, file = file.path(m6_path, "GS_model.rds"))
saveRDS(G_model, file = file.path(m6_path, "G_model.rds"))
