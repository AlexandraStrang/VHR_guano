...
We use the observed number of breeding pairs obtained by drone (UAV) survey for Cape Crozier Ad\'{e}lie penguin colony in 2020 (2020-11-29) as our point locations. 
...
# eventually add Cape Crozier 2019 GA and points
# for model training and testing

```

### 2.2 Model domain (coastline boundary)

Use the boundary of the coastline at Cape Crozier and the bounds of the VHR image to create the boundary (domain) to be used to create INLA (Integrated Nested Laplace Approximation) mesh for the LGCP model. This domain specifies the area to focus the model calculations within (excludes the area beyond the coastline as we know there are no nests present there). We placed an 100 m buffer around the coastline boundary to encapsulate all points and guano area buffer below.
...
### 2.3 Samplers area (VHR-derived guano area)

Use the boundary of the guano area with an 100 m buffer to create the samplers area which is the area that has been 100% sampled for points (penguin nests). The area of guano was derived from VHR imagery at Cape Crozier for 2020. We use the buffered guano area as the samplers area (as opposed to the UAV image boundary) to ensure a consistent method can be used when predictions are made at sites that do not have UAV points.
